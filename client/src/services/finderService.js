...
export async function analyzeFinder(imageBase64, mimeType, objectName) {
  let response;
  try {
    response = await fetch(`${API_BASE}/api/object-finder/search`, {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({ imageBase64, mimeType, objectName }),
    });
  } catch {
